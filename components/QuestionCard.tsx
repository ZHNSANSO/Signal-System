...
const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  if (isChoice || isJudgement) {
    ...
    // For blank and short answer questions
    isCorrect = selectedOptionId === 'CORRECT';
  }

  const [showExplanation, setShowExplanation] = useState(false);
  // For blank/short answer questions: state to control answer reveal
  ...
  const getTypeColor = () => {
    switch (question.type) {
      ...
      case 'short_answer': return 'bg-teal-100 text-teal-700 dark:bg-teal-900 dark:text-teal-300';
  ...
  return (
    ...
    >
      ...
      {/* --- BLANK & SHORT ANSWER INTERFACE --- */}
      {(isBlank || isShortAnswer) && (
          ...
          {!blankRevealed ? (
            <div className="flex flex-col gap-3">
              <div className="p-4 bg-slate-50 dark:bg-slate-700/30 rounded border border-dashed border-slate-300 dark:border-slate-600 text-center text-slate-500 dark:text-slate-400">
                {isShortAnswer ? '请在纸上详细演算步骤' : '请在纸上或心中作答'}
              </div>
              <button 
              ...
              >
                {isShortAnswer ? '查看完整解答过程' : '查看答案 & 解析'}
              </button>
            </div>
          ) : (
             <div className="space-y-3 animate-fadeIn">
               <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
                 <div className="text-xs text-green-600 dark:text-green-400 font-bold uppercase mb-1">参考答案</div>
